...
Obsidianプラグイン **ForgeAI** (`forge-ai`)。Google Gemini AIを統合し、サイドバーチャット、ナレッジ活用、ファイル操作、チャット保存、インラインテキスト補完を提供する。
...
`main.js`, `manifest.json`, `styles.css` をテスト用Vaultの `.obsidian/plugins/forge-ai/` にシンボリックリンクまたはコピーし、Obsidian設定でプラグインを有効化する。開発中の自動リロードには "Hot Reload" コミュニティプラグインを利用。
